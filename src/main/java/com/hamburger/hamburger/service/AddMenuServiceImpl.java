...
public class AddMenuServiceImpl extends PictureBusiness implements IAddMenuService {
    ...
    public void addMenu(@RequestBody HamburgerAddMenuDTO hamburgerAddMenuDTO) {
        log.debug("開始處理添加表單數據業務,參數:{}", hamburgerAddMenuDTO);
        // 創建實體對象
        ...
        BeanUtils.copyProperties(hamburgerAddMenuDTO, hamburgerAddMenu);
        // 執行圖片保存操作
        log.debug("添加圖片的名稱:{}", getAddFileName());
        hamburgerAddMenu.setCommodity(getAddFileName());
        // 將管理員信息寫入到數據庫
    ...
    public String upload(@RequestParam("picFile") MultipartFile picFile) {
        ...
        String addFileName = super.uploadImg(picFile, "upload"); // 調用父類的方法並傳遞方法名
        return addFileName;
    }
}
